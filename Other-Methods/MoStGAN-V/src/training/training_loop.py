...
def training_loop(
        ...
        G_reg_interval=8,  # How often to perform regularization for G? None = disable lazy regularization.
    ...
    if rank == 0:
        if not resume_whole_state:
            ...
            vis.grid_z = torch.randn([vis.labels.shape[0], G.z_dim], device=device).split(
                batch_gpu)  # (num_batches, [batch_size, z_dim])
            vis.grid_c = torch.from_numpy(vis.labels).to(device).split(batch_gpu)  # (num_batches, [batch_size, c_dim])
            vis.grid_t = torch.from_numpy(vis.frames_idx).to(device).split(
                batch_gpu)  # (num_batches, [batch_size, num_frames])
            images = torch.cat([G_ema(z=z, c=c, t=t[:, [0]], noise_mode='const').cpu() for z, c, t in
                                zip(vis.grid_z, vis.grid_c, vis.grid_t)]).numpy()
            ...
            vis.ts = torch.arange(vis.video_len, device=device).float().unsqueeze(0).repeat(vis.num_videos,
                                                                                            1)  # [batch_size, video_len]
        else:
            ...
            images = torch.cat([G_ema(z=z, c=c, t=t[:, [0]], noise_mode='const').cpu() for z, c, t in
                                zip(vis.grid_z, vis.grid_c, vis.grid_t)]).numpy()
            save_image_grid(images, os.path.join(run_dir, f'fakes_resume_{cur_nimg}.jpg'), drange=[-1, 1],
                            grid_size=vis.grid_size)
    else:
        vis = dnnlib.EasyDict()
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            ...
            phase_real_img, phase_real_c, phase_real_t, phase_real_l = batch['image'], batch['label'], batch['times'], \
                                                                       batch['video_len']
            ...
            all_gen_l = [min(training_set.get_video_len(i), G.sampling_dict['max_num_frames']) for i in
                         gen_cond_sample_idx]
            all_gen_t = [sample_frames(G.sampling_dict, use_fractional_t=use_fractional_t_for_G, total_video_len=l) for
                         l in all_gen_l]
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            images = torch.cat([G_ema(z=z, c=c, t=t[:, [0]], noise_mode='const').cpu() for z, c, t in
                                zip(vis.grid_z, vis.grid_c, vis.grid_t)]).numpy()
            save_image_grid(images, os.path.join(run_dir, f'fakes{cur_nimg // 1000:06d}.jpg'), drange=[-1, 1],
                            grid_size=vis.grid_size)

            # Saving videos
            videos_diff_motion = generate_videos(G_ema, vis.vgrid_z, vis.vgrid_c, vis.ts,
                                                 as_grids=True)  # [video_len, 3, h, w]
            if not G_ema.synthesis.motion_encoder is None:
                with torch.no_grad():
                    motion_z = G_ema.synthesis.motion_encoder(c=vis.vgrid_c[[0]], t=vis.ts[[0]])[
                        'motion_z']  # [1, *motion_dims]
                    motion_z = motion_z.repeat_interleave(len(vis.ts), dim=0)  # [batch_size, *motion_dims]
                    videos_same_motion = generate_videos(G_ema, vis.vgrid_z, vis.vgrid_c, vis.ts, motion_z=motion_z,
                                                         as_grids=True)  # [video_len, 3, h, w]
            ...
            io.write_video(os.path.join(run_dir, f'{experiment_name}_videos_{cur_nimg // 1000:06d}.mp4'),
                                       videos_to_save, fps=cfg.dataset.fps, video_codec='h264', options={'crf': '10'})
        ...
        for phase in phases:
            value = []
            if (phase.start_event is not None) and (
                    phase.end_event is not None) and phase.start_event_recorded and phase.end_event_recorded:
