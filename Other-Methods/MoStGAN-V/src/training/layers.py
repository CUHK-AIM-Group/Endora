...
def tensor_modulation_word(weight, styles, normalize: bool = True):
    """
    Performs a low-rank tensor modulation
    weight: [c_out, c_in, kh, kw]
    styles: [b, num_words, rank, c_in + kh + kw]
    """
    b, c_out, c_in, kw, kh = weight.shape
    n_words = styles.shape[1]
    rank = styles.shape[2]

    M = styles[:, :, :, :c_in].view(b, n_words, rank, c_in, 1, 1) \
    ...
    modulation = modulation.view(b, c_out, c_in, kw, kh)

    modulation = modulation / modulation.norm(float('inf'), dim=[1, 2, 3, 4], keepdim=True)
    modulation = F.leaky_relu(modulation, negative_slope=0.2)

    weight = weight.view(b, c_out, c_in, kw, kh)

    return weight * (modulation + 1.0), sim
...
class GenInput(nn.Module):
    def __init__(self, cfg: DictConfig, channel_dim: int, motion_v_dim: int = None):
        ...
        elif self.cfg.input.type == 'series':
            self.input = SeriesInput(self.cfg, channel_dim, motion_v_dim=motion_v_dim)
            self.total_dim = self.input.get_dim()
        else:
            raise NotImplementedError(f'Unkown input type: {self.cfg.input.type}')

    def forward(self, batch_size: int, motion_v: Optional[torch.Tensor] = None, dtype=None,
                memory_format=None) -> torch.Tensor:
        if self.cfg.input.type == 'const':
...
class SeriesInput(nn.Module):
    ...
    def forward(self, motion_v: torch.Tensor) -> torch.Tensor:
        ...
        motion_v: [batch_size, motion_v_dim]
        """
        t = self.cfg.sampling.num_frames_per_video
        bsz = motion_v.shape[0] / t
        motion_v = motion_v.view(-1, t, self.motion_v_dim)
        motion_v = motion_v.unsqueeze(3).unsqueeze(4).repeat(1, 1, 1, *self.const.shape[2:])
        const = self.const.repeat(bsz, 1, 1, 1)
        out = []
        for idx in range(t):
            item = torch.cat([const, motion_v[:, idx].squeeze()], dim=1)
            out.append(item)
            const = const + motion_v[:, idx].squeeze()
        out = torch.stack(out, dim=1)

        misc.assert_shape(out, [bsz, t, None, None, None])

        c_dim = self.get_dim()
        out = out.view(-1, c_dim,  *self.const.shape[2:])
...
class TemporalDifferenceEncoder(nn.Module):
    ...
    def forward(self, t: torch.Tensor) -> torch.Tensor:
        ...
        if self.cfg.sampling.num_frames_per_video == 1:
            ...
            out = out.view(batch_size, num_diffs_to_use, -1).view(batch_size,
                                                                  -1)  # [batch_size, num_diffs_to_use * (d + num_fourier_feats)]
...
class EqLRConv1d(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        y = F.conv1d(input=x, weight=w, bias=b, stride=self.stride,
                     padding=self.padding)  # [batch_size, out_features, out_len]
