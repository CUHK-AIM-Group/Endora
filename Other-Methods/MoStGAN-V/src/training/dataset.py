    ...
    def _load_raw_frames(self, raw_idx: int, frames_idx: List[int] = None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for frame_idx in frames_idx:
            with self._open_file(frame_paths[frame_idx]) as f:
                images.append(load_image_from_buffer(f, resolution=self.reso))
...
def load_image_from_buffer(f, use_pyspng: bool = False, resolution=None) -> np.ndarray:
    if use_pyspng:
        image = pyspng.load(f.read())
    else:
        image = PIL.Image.open(f)
        if resolution is not None:
            image = np.array(image.resize(([resolution, resolution])))
