...
class TransformerBlock(nn.Module):
    """
    A EnDora tansformer block with adaptive layer norm zero (adaLN-Zero) conditioning.
    """
...
class EnDora(nn.Module):
    ...
    def __init__(
        ...
        # self.to_hidden_layer = nn.Linear(in_features=256*384, out_features=64*1152)
    ...
    def forward_with_cfg(self, x, t, y=None, cfg_scale=7.0, use_fp16=False, text_embedding=None):
        """
        Forward pass of EnDora, but also batches the unconditional forward pass for classifier-free guidance.
        """
...
def EnDora_XL_2(**kwargs):
    return EnDora(depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)

def EnDora_XL_2_var(**kwargs):
    return EnDora_var(depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)

def EnDora_XL_4(**kwargs):
    return EnDora(depth=28, hidden_size=1152, patch_size=4, num_heads=16, **kwargs)

def EnDora_XL_8(**kwargs):
    return EnDora(depth=28, hidden_size=1152, patch_size=8, num_heads=16, **kwargs)

def EnDora_L_2(**kwargs):
    return EnDora(depth=24, hidden_size=1024, patch_size=2, num_heads=16, **kwargs)

def EnDora_L_4(**kwargs):
    return EnDora(depth=24, hidden_size=1024, patch_size=4, num_heads=16, **kwargs)

def EnDora_L_8(**kwargs):
    return EnDora(depth=24, hidden_size=1024, patch_size=8, num_heads=16, **kwargs)

def EnDora_B_2(**kwargs):
    return EnDora(depth=12, hidden_size=768, patch_size=2, num_heads=12, **kwargs)

def EnDora_B_4(**kwargs):
    return EnDora(depth=12, hidden_size=768, patch_size=4, num_heads=12, **kwargs)

def EnDora_B_8(**kwargs):
    return EnDora(depth=12, hidden_size=768, patch_size=8, num_heads=12, **kwargs)

def EnDora_S_2(**kwargs):
    return EnDora(depth=12, hidden_size=384, patch_size=2, num_heads=6, **kwargs)

def EnDora_S_4(**kwargs):
    return EnDora(depth=12, hidden_size=384, patch_size=4, num_heads=6, **kwargs)

def EnDora_S_8(**kwargs):
    return EnDora(depth=12, hidden_size=384, patch_size=8, num_heads=6, **kwargs)


EnDora_models = {
    'EnDora-XL/2': EnDora_XL_2,  'EnDora-XL/4': EnDora_XL_4,  'EnDora-XL/8': EnDora_XL_8,
    'EnDora-L/2':  EnDora_L_2,   'EnDora-L/4':  EnDora_L_4,   'EnDora-L/8':  EnDora_L_8,
    'EnDora-B/2':  EnDora_B_2,   'EnDora-B/4':  EnDora_B_4,   'EnDora-B/8':  EnDora_B_8,
    'EnDora-S/2':  EnDora_S_2,   'EnDora-S/4':  EnDora_S_4,   'EnDora-S/8':  EnDora_S_8,
    "EnDora-XL/2_var": EnDora_XL_2_var, 
}
