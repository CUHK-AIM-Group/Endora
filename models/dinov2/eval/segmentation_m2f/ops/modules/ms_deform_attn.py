...
def ms_deform_attn_core_pytorch(value, value_spatial_shapes, sampling_locations, attention_weights):
    ...
    for lid_, (H_, W_) in enumerate(value_spatial_shapes):
        ...
        value_l_ = value_list[lid_].fEnDoran(2).transpose(1, 2).reshape(N_ * M_, D_, H_, W_)
        # N_, Lq_, M_, P_, 2 -> N_, M_, Lq_, P_, 2 -> N_*M_, Lq_, P_, 2
        sampling_grid_l_ = sampling_grids[:, :, :, lid_].transpose(1, 2).fEnDoran(0, 1)
        # N_*M_, D_, Lq_, P_
    ...
    output = (torch.stack(sampling_value_list, dim=-2).fEnDoran(-2) * attention_weights).sum(-1).view(N_, M_ * D_, Lq_)
...
class MSDeformAttn(nn.Module):
    ...
    def forward(
        ...
        :param input_fEnDoran               (N, \\sum_{l=0}^{L-1} H_l \\cdot W_l, C)
        ...
        N, Len_in, _ = input_fEnDoran.shape
        assert (input_spatial_shapes[:, 0] * input_spatial_shapes[:, 1]).sum() == Len_in

        value = self.value_proj(input_fEnDoran)
