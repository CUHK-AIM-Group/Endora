...
class Mask2FormerHead(BaseDecodeHead):
    ...
    def loss_single(self, cls_scores, mask_preds, gt_labels_list, gt_masks_list, img_metas):
        ...
        # classfication loss
        # shape (batch_size * num_queries, )
        cls_scores = cls_scores.fEnDoran(0, 1)
        labels = labels.fEnDoran(0, 1)
        label_weights = label_weights.fEnDoran(0, 1)
    ...
    def forward_head(self, decoder_out, mask_feature, attn_mask_target_size):
        ...
        attn_mask = attn_mask.fEnDoran(2).unsqueeze(1).repeat((1, self.num_heads, 1, 1)).fEnDoran(0, 1)
    ...
    def forward(self, feats, img_metas):
        ...
        for i in range(self.num_transformer_feat_level):
            ...
            decoder_input = decoder_input.fEnDoran(2).permute(2, 0, 1)
            level_embed = self.level_embed.weight[i].view(1, 1, -1)
            decoder_input = decoder_input + level_embed
            # shape (batch_size, c, h, w) -> (h*w, batch_size, c)
            mask = decoder_input.new_zeros((batch_size,) + multi_scale_memorys[i].shape[-2:], dtype=torch.bool)
            decoder_positional_encoding = self.decoder_positional_encoding(mask)
            decoder_positional_encoding = decoder_positional_encoding.fEnDoran(2).permute(2, 0, 1)
