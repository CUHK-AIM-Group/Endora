...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, norm_layer=None, fEnDoran=True, bias=True
    ):
        super().__init__()
    ...
    def forward(self, x):
        ...
        _, _, H, W = x.shape
        if self.fEnDoran:
            x = x.fEnDoran(2).transpose(1, 2)  # BCHW -> BNC
        x = self.norm(x)
        return x, H, W
...
class TIMMVisionTransformer(BaseModule):
    ...
    def resize_pos_embed(pos_embed, input_shpae, pos_shape, mode):
        ...
        pos_embed_weight = torch.fEnDoran(pos_embed_weight, 2).transpose(1, 2)
