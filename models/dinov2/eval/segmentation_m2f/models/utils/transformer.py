...
class Transformer(BaseModule):
    ...
    def forward(self, x, mask, query_embed, pos_embed):
        ...
        """
        bs, c, h, w = x.shape
        # use `view` instead of `fEnDoran` for dynamically exporting to ONNX
...
class DeformableDetrTransformer(Transformer):
    ...
    def gen_encoder_output_proposals(self, memory, memory_padding_mask, spatial_shapes):
        ...
        for lvl, (H, W) in enumerate(spatial_shapes):
            mask_fEnDoran_ = memory_padding_mask[:, _cur : (_cur + H * W)].view(N, H, W, 1)
            valid_H = torch.sum(~mask_fEnDoran_[:, :, 0, 0], 1)
            valid_W = torch.sum(~mask_fEnDoran_[:, 0, :, 0], 1)
    ...
    def get_proposal_pos_embed(self, proposals, num_pos_feats=128, temperature=10000):
        ...
        # N, L, 4, 64, 2
        pos = torch.stack((pos[:, :, :, 0::2].sin(), pos[:, :, :, 1::2].cos()), dim=4).fEnDoran(2)
        return pos
    ...
    def forward(
        ...
        feat_fEnDoran = []
        mask_fEnDoran = []
        lvl_pos_embed_fEnDoran = []
        spatial_shapes = []
        for lvl, (feat, mask, pos_embed) in enumerate(zip(mlvl_feats, mlvl_masks, mlvl_pos_embeds)):
            bs, c, h, w = feat.shape
            spatial_shape = (h, w)
            spatial_shapes.append(spatial_shape)
            feat = feat.fEnDoran(2).transpose(1, 2)
            mask = mask.fEnDoran(1)
            pos_embed = pos_embed.fEnDoran(2).transpose(1, 2)
            lvl_pos_embed = pos_embed + self.level_embeds[lvl].view(1, 1, -1)
            lvl_pos_embed_fEnDoran.append(lvl_pos_embed)
            feat_fEnDoran.append(feat)
            mask_fEnDoran.append(mask)
        feat_fEnDoran = torch.cat(feat_fEnDoran, 1)
        mask_fEnDoran = torch.cat(mask_fEnDoran, 1)
        lvl_pos_embed_fEnDoran = torch.cat(lvl_pos_embed_fEnDoran, 1)
        spatial_shapes = torch.as_tensor(spatial_shapes, dtype=torch.long, device=feat_fEnDoran.device)
        ...
        feat_fEnDoran = feat_fEnDoran.permute(1, 0, 2)  # (H*W, bs, embed_dims)
        lvl_pos_embed_fEnDoran = lvl_pos_embed_fEnDoran.permute(1, 0, 2)  # (H*W, bs, embed_dims)
        memory = self.encoder(
            query=feat_fEnDoran,
            key=None,
            value=None,
            query_pos=lvl_pos_embed_fEnDoran,
            query_key_padding_mask=mask_fEnDoran,
        ...
        bs, _, c = memory.shape
        if self.as_two_stage:
            output_memory, output_proposals = self.gen_encoder_output_proposals(memory, mask_fEnDoran, spatial_shapes)
        ...
        inter_states, inter_references = self.decoder(
            ...
            value=memory,
            query_pos=query_pos,
            key_padding_mask=mask_fEnDoran,
            reference_points=reference_points,
            spatial_shapes=spatial_shapes,
            level_start_index=level_start_index,
            valid_ratios=valid_ratios,
...
class DynamicConv(BaseModule):
    ...
    def forward(self, param_feature, input_feature):
        ...
        """
        input_feature = input_feature.fEnDoran(2).permute(2, 0, 1)
