...
def eval_finetune(args):
    ...
    if args.dataset == "ucf101":
        ...
        dataset_train_u = UCF101(cfg=config, mode="train_u", num_retries=10)
    ...
    train_sampler_u = torch.utils.data.distributed.DistributedSampler(dataset_train_u, shuffle=True)
    ...
    ema_model = get_vit_base_patch16_224(cfg=config, no_head=True)
    ...
    ema_linear_classifier = LinearClassifier(model_embed_dim * (args.n_last_blocks + int(args.avgpool_patchtokens)),
                                            num_labels=args.num_labels)
    for param_q, param_k in zip(linear_classifier.parameters(), ema_linear_classifier.parameters()):
        param_k.data.copy_(param_q.detach().data)  # initialize
        param_k.requires_grad = False  #
    ema_linear_classifier = ema_linear_classifier.cuda()
    ema_linear_classifier.eval()
    ...
    if args.test:
        ...
        test_stats, f1 = validate_network(val_loader, ema_model, ema_linear_classifier, args.n_last_blocks,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train(args, model, linear_classifier, ema_model, ema_linear_classifier, optimizer, train_loader, train_loader_u, epoch, args.n_last_blocks, args.avgpool_patchtokens, prob_list)
        scheduler.step()
        ...
        if epoch % args.val_freq == 0 or epoch == args.epochs - 1:
            ...
            test_stats, f1 = validate_network(val_loader, ema_model, ema_linear_classifier, args.n_last_blocks, args.avgpool_patchtokens)
...
def train(args, model, linear_classifier, ema_model, ema_linear_classifier, optimizer, train_loader, train_loader_u, epoch, n, avgpool, prob_list):
    ...
    for (inp, target, sample_idx, meta) in metric_logger.log_every(train_loader, 20, header):
        # move to gpu
        inp = inp.cuda(non_blocking=True) # (bz, 3, 8, 224, 224)
        target = target.cuda(non_blocking=True) # (bz,) 0 0 1 1

        try:
            data = next(train_loader_u_iter)
            (inp_u, inp_u_aug, sample_idx_u, meta_u) = data
            # 在这里使用获取到的数据
            # inp_u, sample_idx_u, meta_u 分别是无标签数据的输入、样本索引和元数据
        except StopIteration:
            train_loader_u_iter = iter(train_loader_u)
            data = next(train_loader_u_iter)
            (inp_u, inp_u_aug, sample_idx_u, meta_u) = data

        inp_u = inp_u.cuda(non_blocking=True) # (bz, 3, 8, 224,224)
        inp_u_aug = inp_u_aug.cuda(non_blocking=True) # (bz, 3, 8, 224,224)

        inp_cat = torch.cat([
            inp, inp_u, inp_u_aug
        ])

        output_cat = model(inp_cat)
        output_cat = linear_classifier(output_cat)

        output_x = output_cat[:inp.size(0)]
        output_u = output_cat[inp.size(0):inp.size(0) + inp_u.size(0)]
        output_u_aug = output_cat[inp.size(0) + inp_u.size(0):]

        # output = model(inp)
        # output = linear_classifier(output)

        # output_u = model(inp_u)
        # output_u = linear_classifier(output_u)


        # compute cross entropy loss
        loss_x = nn.CrossEntropyLoss()(output_x, target)

        with torch.no_grad():
            probs = torch.softmax(output_u, dim=1)
        ...
        loss_u = (nn.CrossEntropyLoss(reduction='none')(output_u, lbs_u_guess) * mask).mean()
        ...
        with torch.no_grad():
            ema_model_update(model, linear_classifier, ema_model, ema_linear_classifier, 0.999)
...
@torch.no_grad()
def ema_model_update(model, linear_classifier, ema_model, ema_linear_classifier, ema_m):
    """
    ...
    for param_train, param_eval in zip(linear_classifier.parameters(), ema_linear_classifier.parameters()):
        param_eval.copy_(param_eval * ema_m + param_train.detach() * (1-ema_m))
    
    for buffer_train, buffer_eval in zip(linear_classifier.buffers(), ema_linear_classifier.buffers()):
        buffer_eval.copy_(buffer_train)
...
if __name__ == '__main__':
    ...
    # paraser.add_argument('--u_dir', type=str, default='col210_5_128_latte_video35W_200')
