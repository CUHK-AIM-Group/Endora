...
import datasets.randaugment as randaugment
...
def spatial_sampling(
    ...
    if spatial_idx == -1:
        ...
        if strong_aug:
            # choice = np.random.choice(0,5) # choose one of the 5 augmentations
            # if choice == 0:
            #     frames = transfom.autocontrast_func(frames)
            # elif choice == 1:
            #     frames = transform.equalize_func(frames)

            min_v  = frames.min()
            max_v  = frames.max()
            uint8_frames = (frames - min_v) / (max_v - min_v) *255
            uint8_frames = uint8_frames.numpy().astype(np.uint8)
            frames_aug = torch.tensor(
                randaugment.RA_func(
                uint8_frames )
                )
            frames_aug = frames_aug*(max_v - min_v)/255.0 + min_v
            frames_aug = torch.tensor(frames_aug) 
            return frames, frames_aug
        else:
            return frames
