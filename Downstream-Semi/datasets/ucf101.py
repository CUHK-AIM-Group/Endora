...
class UCF101(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode, num_retries=10):
        ...
        assert mode in ["train", "val", "test", "train_u"], "Split '{}' not supported for UCF101".format(mode)
        self.mode = mode
        self.cfg = cfg
        ...
        if self.mode in ["train", "train_u"]:  # len=173
            self._num_clips = 1
        ...
        print("Constructing {}...".format(mode))
    ...
    def _construct_loader(self):
        ...
        if self.mode == 'train_u':
            ...
            path_to_file = os.path.join(
                self.cfg.DATA.PATH_TO_GEN_DATA_DIR, "train.txt"
            )
        else:
            path_to_file = os.path.join(
        ...
        if self.mode== 'train':
            ...
            self.used_path_to_videos = []
            self.used_labels = []
            self.used_spatial_temporal_idx = []
            
            for i in range( len(self._labels) ): 
                if self._labels[i] == 1 and cnt_cls_1 < n_label//2:
                    self.used_path_to_videos.append(self._path_to_videos[i])
                    self.used_labels.append(self._labels[i])
                    self.used_spatial_temporal_idx.append(self._spatial_temporal_idx[i])
                    cnt_cls_1 += 1
                
                if self._labels[i] == 0 and cnt_cls_0 < n_label//2:
                    self.used_path_to_videos.append(self._path_to_videos[i])
                    self.used_labels.append(self._labels[i])
                    self.used_spatial_temporal_idx.append(self._spatial_temporal_idx[i])
                    cnt_cls_0 += 1
            ...
            self._path_to_videos = self.used_path_to_videos
            self._labels = self.used_labels
            self._spatial_temporal_idx = self.used_spatial_temporal_idx
    ...
    def __getitem__(self, index):
        ...
        for i_try in range(self._num_retries): # 10
            ...
            if self.mode == 'train_u':
                frames, frames_aug = spatial_sampling(
                    frames,
                    spatial_idx=spatial_sample_index,
                    min_scale=min_scale, # 256
                    max_scale=max_scale, # 320
                    crop_size=crop_size, # 224
                    random_horizontal_flip=self.cfg.DATA.RANDOM_FLIP,
                    inverse_uniform_sampling=self.cfg.DATA.INV_UNIFORM_SAMPLE,
                    strong_aug=True
                )
                return frames, frames_aug, index, {}
